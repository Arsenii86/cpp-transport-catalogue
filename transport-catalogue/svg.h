...
inline std::ostream& operator<<(std::ostream& stream, const Color& color){
      ...
      if (holds_alternative<std::string>(color)) {
        stream << get<std::string>(color);
       }
    
       if (holds_alternative<Rgb>(color)) {          
           const auto& rgb=get<Rgb>(color);           
           stream  << "rgb("sv <<std::to_string(rgb.red) << ','<< std::to_string(rgb.green)<<','<< std::to_string(rgb.blue)<<')';           
        }
    
      if (holds_alternative<Rgba>(color)) {
       const auto& rgba=get<Rgba>(color);
       stream  << "rgba("sv << std::to_string(rgba.red) << ','<< std::to_string(rgba.green) <<','<< std::to_string(rgba.blue)<<','<< rgba.opacity<<')';
      }
    return stream; 
};
...
protected:
    ...
    void RenderAttrs(std::ostream& out) const {
        ...
        if (fill_color_!=std::nullopt) {
            out << "fill=\""sv;
            std::visit([&out,this](auto value) {this->PrintColor(out, value);}, *fill_color_);
            out << "\""sv;   
        }
        if (stroke_color_!=std::nullopt) {           
            out << " stroke=\""sv ;
            std::visit([&out,this](auto value) {this->PrintColor(out, value);}, *stroke_color_);
            out << "\""sv;          
        }
        if (stroke_width_) {
...
private:
    Owner& AsOwner() {
     ...
     void PrintColor( std::ostream& out,std:: monostate)const  {
        out << "none"sv ;
    }
     void PrintColor( std::ostream& out, std::string str)const  {
        out << str ;
    }
     void PrintColor( std::ostream& out, svg::Rgb rgb)const {
        out << "rgb("sv << std::to_string(rgb.red) << ','<< std::to_string(rgb.green)<<','<< std::to_string(rgb.blue)<<')';
    }
     void PrintColor( std::ostream& out, svg::Rgba rgba)const {
